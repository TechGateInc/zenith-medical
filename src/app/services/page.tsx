          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {primaryServices.map((service, index) => {
                // Filter out features we want to remove
                const filteredFeatures = service.features.filter(
                  (feature) =>
                    !(
                      feature.toLowerCase().includes('general health check') ||
                      feature.toLowerCase().includes('immunization') ||
                      feature.toLowerCase().includes('mental health')
                    )
                );
                // If all features are removed, skip rendering this service
